...
namespace Colyseus
{
	...
	public class Client
	{
		...
		public WebSocket ws;
		private IDictionary<string, BaseRoom> rooms = new Dictionary<string, BaseRoom>();
		...
		void ParseMessage(byte[] recv)
		{
			...
			else if (code == Protocol.ROOM_STATE)
			{
				...
				// var state = ((JObject)message[2]).ToObject<ChatRoom>();
				...
				room.SetState((JObject)message[2], remoteCurrentTime, remoteElapsedTime);
			}
		...
		public Room<T> Join<T>(string roomName, object options = null) where T : class
		{
			if (!this.rooms.ContainsKey(roomName))
			{
				var newRoom = new Room<T>(this, roomName);
				this.rooms.Add(roomName, newRoom);
			}

			LoginGUI.Log("Join room: " + roomName);

			this.Send(new object[] { Protocol.JOIN_ROOM, roomName, options });

			return (Room<T>)this.rooms[roomName];
		}
